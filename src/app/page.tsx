...
import { Send, Leaf, MapPin, Users, Clock, Zap, ThermometerSun, Search, X } from 'lucide-react';
...
export default function RoomScheduler() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: 'Hi! I can help you find the perfect room. Tell me what date and time you need it! 🌱' }
  ]);
  ...
  const buildings = ['all', 'Science Center', 'Library', 'Music Building', 'Tech Hub', 'Arts Building'];
      ...
      <div className="w-2/5 border-r border-emerald-200 bg-white flex flex-col shadow-lg">
        ...
        <div className="p-4 bg-emerald-50 border-b border-emerald-200 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-emerald-700 mb-1 block">Date</label>
              <input
                type="date"
                value={selectedDate}
            ...
            <div>
              <label className="text-xs font-semibold text-emerald-700 mb-1 block">Time</label>
              <select
                value={selectedTime}
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                    room.available ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                  }`}>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${getEfficiencyColor(room.energyEfficiency)}`}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {selectedRoom && (
          <div className="bg-white border-b border-emerald-200 p-6 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-3xl font-bold text-gray-800">{selectedRoom.name}</h2>
            ...
            <div className="grid grid-cols-4 gap-4 mb-4">
              <div className="bg-emerald-50 p-4 rounded-lg border-2 border-emerald-200">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-bold text-emerald-700">Sustainability</span>
                </div>
                <p className="text-2xl font-bold text-emerald-700">{selectedRoom.sustainabilityScore}</p>
              </div>

              <div className="bg-emerald-50 p-4 rounded-lg border-2 border-emerald-200">
                <div className="flex items-center gap-2 mb-1">
                  <Zap className="w-5 h-5 text-emerald-600" />
                  <span className="text-xs font-bold text-emerald-700">Cleanliness</span>
                </div>
                <p className="text-2xl font-bold text-emerald-700">{selectedRoom.cleanlinessScore}</p>
              </div>

              <div className="bg-emerald-50 p-4 rounded-lg border-2 border-emerald-200">
                <div className="flex items-center gap-2 mb-1">
                  <ThermometerSun className="w-5 h-5 text-emerald-600" />
                  <span className="text-xs font-bold text-emerald-700">Efficiency</span>
                </div>
                <p className="text-2xl font-bold text-emerald-700">{selectedRoom.energyEfficiency}</p>
              </div>

              <div className="bg-emerald-50 p-4 rounded-lg border-2 border-emerald-200">
                <div className="flex items-center gap-2 mb-1">
                  <Users className="w-5 h-5 text-emerald-600" />
                  <span className="text-xs font-bold text-emerald-700">Occupancy</span>
                </div>
                <p className="text-2xl font-bold text-emerald-700">{selectedRoom.occupancy}</p>
              </div>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-gradient-to-b from-white to-emerald-50">
          <div className="p-4 border-b border-emerald-200 bg-white">
            ...
            <p className="text-sm text-gray-600">Ask me about room availability, sustainability, and booking!</p>
          </div>
      ...
      {showBookingModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Book {selectedRoom?.name}</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Purpose (Optional)</label>
                <textarea
              ...
              <div className="bg-emerald-50 p-4 rounded-lg border-2 border-emerald-200">
                <p className="text-sm text-gray-700"><strong>Room:</strong> {selectedRoom?.name}</p>
                <p className="text-sm text-gray-700"><strong>Building:</strong> {selectedRoom?.building}</p>
                <p className="text-sm text-gray-700"><strong>Date:</strong> {selectedDate && new Date(selectedDate + 'T00:00:00').toLocaleDateString()}</p>
